...
export async function seedMunicipioOptions(
  ...
  if (!municipioQuestion) {
    console.warn(
      '[seed:municipio] Pregunta Municipio no encontrada — saltando seed.',
    );
    return;
  }
  ...
  for (const [deptName, townNames] of Object.entries(MUNICIPIOS)) {
    ...
    if (!dept) {
      console.warn(
        `[seed:municipio] Departamento "${deptName}" no encontrado en DB — saltando.`,
      );
      continue;
    }
    ...
    console.log(
      `[seed:municipio] ${deptName}: towns y opciones sincronizados.`,
    );
  }
  ...
  for (const option of deptOptions) {
    ...
    if (option.metadataId !== dept.departmentId) {
      ...
      await optionRepo.save(option);
      console.log(
        `[seed:municipio] Departamento "${option.text}" → metadata_id asignado.`,
      );
    }
  }
